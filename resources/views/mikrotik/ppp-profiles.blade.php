                                    ...
                                    <div class="d-flex justify-content-center gap-1">
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end" style="font-size:0.8rem;min-width:160px;">
                                                <li><a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#editPppModal"
                                                    data-id="{{ $p['.id'] ?? '' }}"
                                                    data-name="{{ $p['name'] ?? '' }}"
                                                    data-rate-limit="{{ $p['rate-limit'] ?? '' }}"><i class="fa-solid fa-pen me-2 text-primary"></i>Edit</a></li>
                                                <li><hr class="dropdown-divider"></li>
                                                <li>
                                                    <form method="POST" action="{{ route('mikrotik.ppp-profiles.destroy', $p['.id'] ?? '') }}" onsubmit="return confirm('Hapus profile {{ $p['name'] }}?')">
                                                        @csrf @method('DELETE')
