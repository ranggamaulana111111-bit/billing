...
@section('title', 'Raw Firewall — Security Policy Center')
    ...
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-1" style="font-size:0.78rem;">
                ...
                <li class="breadcrumb-item active">Raw Firewall</li>
            </ol>
        </nav>
        <h2 class="mb-0"><i class="fa-solid fa-fire me-2" style="color:#6366f1;"></i>Raw Firewall</h2>
...
<table class="table table-hover align-middle mb-0 mon-table">
                ...
                <tbody>
                    ...
                    <tr><td colspan="12" class="text-center text-muted py-4">
                        <i class="fa-solid fa-fire" style="font-size:2rem;"></i>
                        <p class="mt-2 mb-0">No raw firewall rules found</p>
                    </td></tr>
                    @endforelse
                </tbody>
            </table>
...
<div class="modal fade" id="createModal" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content">
    <form method="POST" action="{{ route('noc.security.raw-store') }}">
        @csrf
        <input type="hidden" name="router_id" value="{{ $router->id ?? '' }}">
        <div class="modal-header"><h5 class="modal-title fw-bold">Add Raw Firewall Rule</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
...
<div class="modal fade" id="editModal" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content">
    <form method="POST" id="editForm" action="">
        @csrf @method('PUT')
        <input type="hidden" name="router_id" value="{{ $router->id ?? '' }}">
        <div class="modal-header"><h5 class="modal-title fw-bold">Edit Raw Firewall Rule</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <div class="modal-body">
            <div class="row g-3">
...
<form method="POST" id="bulkForm" action="{{ route('noc.security.raw-bulk') }}" style="display:none;">@csrf
    <input type="hidden" name="router_id" value="{{ $router->id ?? '' }}">
...
@push('scripts')
<script>
const ROUTE_UPDATE = @json(route('noc.security.raw-update', ['itemId' => 'X']));
const ROUTE_DESTROY = @json(route('noc.security.raw-destroy', ['itemId' => 'X']));
const ROUTE_TOGGLE = @json(route('noc.security.raw-toggle', ['itemId' => 'X']));
const ROUTE_COPY = @json(route('noc.security.raw-copy', ['itemId' => 'X']));
