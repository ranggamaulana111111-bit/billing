...
@section('title', 'Firewall Filter — Security Policy Center')
    ...
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-1" style="font-size:0.78rem;">
                ...
                <li class="breadcrumb-item active">Firewall Filter</li>
            </ol>
        </nav>
        <h2 class="mb-0"><i class="fa-solid fa-filter me-2" style="color:#ef4444;"></i>Firewall Filter</h2>
        <p class="section-subtitle mb-0 mt-1">{{ $router->display_identity ?? 'No router' }} · {{ count($items) }} rules</p>
    </div>
...
<table class="table table-hover align-middle mb-0 mon-table">
                ...
                <tbody>
                    ...
                    <tr data-chain="{{ $item['chain'] ?? '' }}" data-action="{{ $item['action'] ?? '' }}" data-status="{{ $disabled ? 'disabled' : ($dynamic ? 'dynamic' : 'enabled') }}" data-search="{{ strtolower(implode(' ', [$item['chain'] ?? '', $item['action'] ?? '', $item['src-address'] ?? '', $item['dst-address'] ?? '', $item['dst-port'] ?? '', $item['comment'] ?? ''])) }}">
                        ...
                        <td>
                            @php $actColors = ['accept'=>'success','drop'=>'danger','reject'=>'warning','log'=>'info','tarpit'=>'dark']; @endphp
                    ...
                    <tr><td colspan="12" class="text-center text-muted py-4">
                        ...
                        <p class="mt-2 mb-0">No firewall filter rules found</p>
                    </td></tr>
                    @endforelse
                </tbody>
            </table>
...
<div class="modal fade" id="createModal" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content">
    <form method="POST" action="{{ route('noc.security.firewall-filter-store') }}">
        @csrf
        <input type="hidden" name="router_id" value="{{ $router->id ?? '' }}">
        <div class="modal-header"><h5 class="modal-title fw-bold">Add Firewall Filter Rule</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <div class="modal-body">
            <div class="row g-3">
                <div class="col-md-4"><label class="form-label fw-semibold">Chain <span class="text-danger">*</span></label>
                    <select name="chain" class="form-select" required><option value="input">input</option><option value="forward">forward</option><option value="output">output</option></select>
                </div>
                <div class="col-md-4"><label class="form-label fw-semibold">Action <span class="text-danger">*</span></label>
                    <select name="action" class="form-select" required><option value="accept">accept</option><option value="drop">drop</option><option value="reject">reject</option><option value="log">log</option><option value="tarpit">tarpit</option></select>
                </div>
                ...
                <div class="col-md-4"><label class="form-label fw-semibold">Src Address List</label><input type="text" name="src-address-list" class="form-control"></div>
                <div class="col-md-4"><label class="form-label fw-semibold">Dst Address List</label><input type="text" name="dst-address-list" class="form-control"></div>
                <div class="col-md-4"><label class="form-label fw-semibold">Connection State</label><input type="text" name="connection-state" class="form-control" placeholder="established,related"></div>
...
<div class="modal fade" id="editModal" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content">
    <form method="POST" id="editForm" action="">
        @csrf @method('PUT')
        <input type="hidden" name="router_id" value="{{ $router->id ?? '' }}">
        <div class="modal-header"><h5 class="modal-title fw-bold">Edit Firewall Filter Rule</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <div class="modal-body">
            <div class="row g-3">
                <div class="col-md-4"><label class="form-label fw-semibold">Chain</label><select name="chain" id="editChain" class="form-select"><option value="input">input</option><option value="forward">forward</option><option value="output">output</option></select></div>
                <div class="col-md-4"><label class="form-label fw-semibold">Action</label><select name="action" id="editAction" class="form-select"><option value="accept">accept</option><option value="drop">drop</option><option value="reject">reject</option><option value="log">log</option><option value="tarpit">tarpit</option></select></div>
                ...
                <div class="col-md-4"><label class="form-label fw-semibold">Src Address List</label><input type="text" name="src-address-list" id="editSrcAddrList" class="form-control"></div>
                <div class="col-md-4"><label class="form-label fw-semibold">Dst Address List</label><input type="text" name="dst-address-list" id="editDstAddrList" class="form-control"></div>
                <div class="col-md-4"><label class="form-label fw-semibold">Connection State</label><input type="text" name="connection-state" id="editConnState" class="form-control"></div>
...
<form method="POST" id="bulkForm" action="{{ route('noc.security.firewall-filter-bulk') }}" style="display:none;">@csrf
    <input type="hidden" name="router_id" value="{{ $router->id ?? '' }}">
...
@push('scripts')
<script>
const ROUTE_UPDATE = @json(route('noc.security.firewall-filter-update', ['itemId' => 'X']));
const ROUTE_DESTROY = @json(route('noc.security.firewall-filter-destroy', ['itemId' => 'X']));
const ROUTE_TOGGLE = @json(route('noc.security.firewall-filter-toggle', ['itemId' => 'X']));
const ROUTE_COPY = @json(route('noc.security.firewall-filter-copy', ['itemId' => 'X']));
...
function prefillEdit(item) {
    ...
    document.getElementById('editChain').value = item['chain'] || 'input';
    document.getElementById('editAction').value = item['action'] || 'accept';
    ...
    document.getElementById('editSrcAddrList').value = item['src-address-list'] || '';
    document.getElementById('editDstAddrList').value = item['dst-address-list'] || '';
    document.getElementById('editConnState').value = item['connection-state'] || '';
