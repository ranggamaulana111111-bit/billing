...
@section('title', 'Hotspot User Manager — NOC')
    ...
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-1" style="font-size:0.78rem;">
                ...
                <li class="breadcrumb-item active">Hotspot User</li>
            </ol>
        </nav>
        <h2 class="mb-0">
            <i class="fa-solid fa-users me-2" style="color:var(--primary);"></i>Hotspot User Manager
        </h2>
    </div>
...
<div class="card border-0 shadow-sm mb-4">
    <div class="card-body py-3">
        <form method="GET" class="row g-2 align-items-end">
            ...
            <div class="col-md-4">
                <input type="text" id="searchInput" class="form-control form-control-sm" placeholder="Search users..." onkeyup="filterTable()">
            </div>
        ...
        <div class="card shadow-sm border-0 h-100">
            <div class="card-body">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <div class="text-muted" style="font-size:0.78rem;">Total Users</div>
            ...
            <div class="card-body">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <h4 class="mb-0 fw-bold">{{ $error ? '—' : count($activeSessions ?? []) }}</h4>
                    </div>
                </div>
            </div>
        </div>
...
<table class="table table-hover align-middle mb-0 mon-table">
                ...
                <tbody>
                            ...
                            <div class="d-flex justify-content-center gap-1">
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end" style="font-size:0.8rem;min-width:160px;">
                                        ...
                                        <li>
                                            <form method="POST" action="{{ route('noc.internet.hotspotuser-toggle', $item['.id'] ?? '') }}">
                                                @csrf
                    ...
                    <tr><td colspan="8" class="text-center text-muted py-4">
                        ...
                        <p class="mt-2 mb-0">No hotspot users found on this router</p>
                    </td></tr>
                    @endforelse
                </tbody>
            </table>
...
<div class="modal fade" id="createModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <form method="POST" action="{{ route('noc.internet.hotspotuser-store') }}">
        @csrf
        <input type="hidden" name="router_id" value="{{ $router->id }}">
        <div class="modal-header"><h5 class="modal-title fw-bold">Add Hotspot User</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <div class="modal-body">
            ...
            <div class="mb-3"><label class="form-label fw-semibold">Server</label><input type="text" name="server" class="form-control" placeholder="all"></div>
            ...
            <div class="mb-3"><label class="form-label fw-semibold">Limit Uptime</label><input type="text" name="limit-uptime" class="form-control" placeholder="e.g. 1d00:00:00"></div>
...
<div class="modal fade" id="editModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <form method="POST" id="editForm" action="">
        ...
        @method('PUT')
        <input type="hidden" name="router_id" value="{{ $router->id }}">
        <div class="modal-header"><h5 class="modal-title fw-bold">Edit Hotspot User</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <div class="modal-body">
            ...
            <div class="mb-3"><label class="form-label fw-semibold">Limit Uptime</label><input type="text" name="limit-uptime" id="editLimitUptime" class="form-control" placeholder="e.g. 1d00:00:00"></div>
...
{{-- ═══ BULK FORM ═══ --}}
<form method="POST" id="bulkForm" action="{{ route('noc.internet.hotspotuser-bulk') }}" style="display:none;">@csrf
    <input type="hidden" name="router_id" value="{{ $router->id }}">
...
@push('scripts')
<script>
const ROUTE_UPDATE = @json(route('noc.internet.hotspotuser-update', ['itemId' => 'X']));
const ROUTE_DESTROY = @json(route('noc.internet.hotspotuser-destroy', ['itemId' => 'X']));
...
function prefillEdit(item) {
    ...
    document.getElementById('editLimitUptime').value = item['limit-uptime'] || '';
    document.getElementById('editComment').value = item['comment'] || '';
}

function deleteItem(id, name) {
    if (!confirm('Delete hotspot user "' + name + '"? This action cannot be undone.')) return;
