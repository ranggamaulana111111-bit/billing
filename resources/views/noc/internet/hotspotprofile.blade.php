...
@section('title', 'Hotspot Profile Manager — NOC')
    ...
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-1" style="font-size:0.78rem;">
                ...
                <li class="breadcrumb-item active">Hotspot Profile</li>
            </ol>
        </nav>
        <h2 class="mb-0">
            <i class="fa-solid fa-id-badge me-2" style="color:var(--primary);"></i>Hotspot Profile Manager
        </h2>
    </div>
            ...
            <div class="card-body">
                <div class="d-flex align-items-center">
                    <div class="rounded-circle d-flex align-items-center justify-content-center me-3" style="width:48px;height:48px;background:rgba(37,99,235,0.1);">
                        <i class="fa-solid fa-id-badge fa-lg" style="color:#2563eb;"></i>
                    </div>
                    <div>
...
<table class="table table-hover align-middle mb-0 mon-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><code style="font-size:0.78rem;">{{ $item['session-timeout'] ?? '—' }}</code></td>
                        <td class="text-muted">{{ $item['comment'] ?? '—' }}</td>
                        <td>
                            <div class="btn-group btn-group-sm">
                                <button class="btn btn-outline-primary py-0" title="Edit" data-bs-toggle="modal" data-bs-target="#editModal" onclick="prefillEdit({{ json_encode($item) }})">
                                    <i class="fa-solid fa-pen"></i>
                                </button>
                                <button class="btn btn-outline-danger py-0" title="Delete" onclick="deleteItem('{{ $item['.id'] }}', '{{ addslashes($item['name'] ?? '') }}')">
                    ...
                    <tr><td colspan="6" class="text-center text-muted py-4">
                        ...
                        <p class="mt-2 mb-0">No hotspot profiles found on this router</p>
                    </td></tr>
                    @endforelse
                </tbody>
            </table>
...
<div class="modal fade" id="createModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <form method="POST" action="{{ route('noc.internet.hotspotprofile-store') }}">
        @csrf
        <input type="hidden" name="router_id" value="{{ $router->id }}">
        <div class="modal-header"><h5 class="modal-title fw-bold">Add Hotspot Profile</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <div class="modal-body">
            ...
            <div class="mb-3"><label class="form-label fw-semibold">Rate Limit</label><input type="text" name="rate-limit" class="form-control" placeholder="e.g. 10M/5M"></div>
            <div class="mb-3"><label class="form-label fw-semibold">Session Timeout</label><input type="text" name="session-timeout" class="form-control" placeholder="e.g. 1d00:00:00"></div>
...
<div class="modal fade" id="editModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <form method="POST" id="editForm" action="">
        ...
        @method('PUT')
        <input type="hidden" name="router_id" value="{{ $router->id }}">
        <div class="modal-header"><h5 class="modal-title fw-bold">Edit Hotspot Profile</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <div class="modal-body">
            ...
            <div class="mb-3"><label class="form-label fw-semibold">Rate Limit</label><input type="text" name="rate-limit" id="editRateLimit" class="form-control" placeholder="e.g. 10M/5M"></div>
            <div class="mb-3"><label class="form-label fw-semibold">Session Timeout</label><input type="text" name="session-timeout" id="editSessionTimeout" class="form-control" placeholder="e.g. 1d00:00:00"></div>
...
{{-- ═══ BULK FORM ═══ --}}
<form method="POST" id="bulkForm" action="{{ route('noc.internet.hotspotprofile-bulk', ['resource' => 'hotspot-profile']) }}" style="display:none;">@csrf
    <input type="hidden" name="router_id" value="{{ $router->id }}">
...
@push('scripts')
<script>
const ROUTE_UPDATE = @json(route('noc.internet.hotspotprofile-update', ['itemId' => 'X']));
const ROUTE_DESTROY = @json(route('noc.internet.hotspotprofile-destroy', ['itemId' => 'X']));
...
function prefillEdit(item) {
    ...
    document.getElementById('editSessionTimeout').value = item['session-timeout'] || '';
