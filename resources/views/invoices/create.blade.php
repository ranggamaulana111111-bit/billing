...
<div class="page-header d-flex flex-wrap justify-content-between align-items-center">
    <div>
        <h2 class="mb-0"><i class="fa-solid fa-plus me-2" style="color:var(--primary);"></i>Buat Tagihan</h2>
        <p class="section-subtitle mb-0 mt-1">Buat tagihan manual untuk pelanggan</p>
    </div>
</div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="customer_id" id="customer_id" class="form-select @error('customer_id') is-invalid @enderror" required>
                            ...
                            @foreach($customers as $c)
                                <option value="{{ $c->id }}" data-price="{{ $c->package->price ?? 0 }}" {{ old('customer_id') == $c->id ? 'selected' : '' }}>
                                    {{ $c->name }} — {{ $c->package->name ?? '-' }} ({{ $c->phone }})
                                </option>
                            @endforeach
                        </select>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    customerSelect.addEventListener('change', autoFillAmount);
