...
<div class="page-header d-flex flex-wrap justify-content-between align-items-center">
    ...
    <div class="page-actions mt-2 mt-md-0 d-flex gap-2">
        ...
        <button type="button" class="btn btn-primary px-3 py-2" data-bs-toggle="modal" data-bs-target="#createPointModal" {{ $routes->isEmpty() ? 'disabled' : '' }}>
            <i class="fa-solid fa-location-dot me-1"></i>Tambah Titik ODP
        </button>
    </div>
</div>
            ...
            <table class="table mb-0 align-middle">
                ...
                <tbody>
                    @forelse($odcs as $odc)
                        <tr>
                            <td>
                                <div class="fw-semibold">{{ $odc->name }}</div>
                                @if($odc->notes)
                                    <small class="text-muted">{{ $odc->notes }}</small>
                                @endif
                            </td>
                            <td class="text-muted small">{{ $odc->address ?? '-' }}</td>
                            <td class="text-center">{{ $odc->capacity }}</td>
                            <td class="text-center">{{ $odc->routes_count }}</td>
                            <td class="text-center">
                                @php
                                    $odcStatus = match($odc->status) {
                                        'active' => ['bg' => '#f0fdf4', 'color' => '#059669', 'label' => 'Aktif'],
                                        'maintenance' => ['bg' => '#fff7ed', 'color' => '#d97706', 'label' => 'Maintenance'],
                                        'inactive' => ['bg' => '#f1f5f9', 'color' => '#64748b', 'label' => 'Nonaktif'],
                                        default => ['bg' => '#f1f5f9', 'color' => '#64748b', 'label' => ucfirst($odc->status)],
                                    };
                                @endphp
                                <span class="badge badge-premium" style="background:{{ $odcStatus['bg'] }};color:{{ $odcStatus['color'] }};">{{ $odcStatus['label'] }}</span>
                            </td>
                            <td class="text-center">
                                ...
                                <form method="POST" action="{{ route('distribution.odcs.destroy', $odc) }}" class="d-inline" onsubmit="return confirm('Hapus ODC {{ $odc->name }}?')">
                                    @csrf @method('DELETE')
                ...
                <tbody>
                    @forelse($odps as $o)
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                @php
                                    $statusStyle = match($o->status) {
                                        'active' => ['bg' => '#f0fdf4', 'color' => '#059669', 'label' => 'Aktif'],
                                        'maintenance' => ['bg' => '#fff7ed', 'color' => '#d97706', 'label' => 'Maintenance'],
                                        'inactive' => ['bg' => '#f1f5f9', 'color' => '#64748b', 'label' => 'Nonaktif'],
                                        default => ['bg' => '#f1f5f9', 'color' => '#64748b', 'label' => ucfirst($o->status)],
                                    };
                                @endphp
                                <span class="badge badge-premium" style="background:{{ $statusStyle['bg'] }};color:{{ $statusStyle['color'] }};">{{ $statusStyle['label'] }}</span>
                            </td>
                            <td class="text-center">
                                <button type="button" class="btn btn-sm btn-outline-primary px-2" data-bs-toggle="modal" data-bs-target="#editPointModal{{ $o->id }}" title="Edit titik">
                                    <i class="fa-solid fa-pen"></i>
                                </button>
                                <form method="POST" action="{{ route('distribution.points.destroy', $o) }}" class="d-inline" onsubmit="return confirm('Hapus titik ODP {{ $o->name }}?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-outline-danger px-2" title="Hapus titik">
                                        <i class="fa-solid fa-trash"></i>
                                    </button>
                                </form>
                            </td>
            ...
            <form method="POST" action="{{ route('distribution.odcs.store') }}">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control" placeholder="contoh: ODC Kumpay Utama" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Alamat</label>
                        <input type="text" name="address" class="form-control" placeholder="Lokasi ODC">
                    </div>
                    <div class="row g-3">
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold">Latitude</label>
                            <input type="number" step="0.0000001" name="latitude" class="form-control" placeholder="-6.4760000">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold">Longitude</label>
                            <input type="number" step="0.0000001" name="longitude" class="form-control" placeholder="106.0140000">
                        </div>
                    </div>
                    <div class="row g-3">
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold">Kapasitas Core/Port</label>
...
<div class="modal fade" id="createPointModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ route('distribution.points.store') }}">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title">Tambah Titik ODP</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control" placeholder="contoh: ODP-001" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Alamat</label>
                        <input type="text" name="address" class="form-control" placeholder="Lokasi titik ODP">
                    </div>
                    <div class="row g-3">
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold">Latitude</label>
                            <input type="number" step="0.0000001" name="latitude" class="form-control" placeholder="-6.4760000" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold">Longitude</label>
                            <input type="number" step="0.0000001" name="longitude" class="form-control" placeholder="106.0140000" required>
                        </div>
                    </div>
                    <div class="row g-3">
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold">Kapasitas Port</label>
                            <input type="number" name="port_capacity" class="form-control" value="8" min="1" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold">Status</label>
                            <select name="status" class="form-select" required>
            ...
            <form method="POST" action="{{ route('distribution.odcs.update', $odc) }}">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control" value="{{ $odc->name }}" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Alamat</label>
                        <input type="text" name="address" class="form-control" value="{{ $odc->address }}">
                    </div>
                    <div class="row g-3">
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Latitude</label>
                            <input type="number" step="0.0000001" name="latitude" class="form-control" value="{{ $odc->latitude }}">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Longitude</label>
                            <input type="number" step="0.0000001" name="longitude" class="form-control" value="{{ $odc->longitude }}">
                        </div>
                    </div>
                    <div class="row g-3">
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Kapasitas Core/Port</label>
                            <input type="number" name="capacity" class="form-control" value="{{ $odc->capacity }}" min="0" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Status</label>
                            <select name="status" class="form-select" required>
                                <option value="active" {{ $odc->status === 'active' ? 'selected' : '' }}>Aktif</option>
                                <option value="maintenance" {{ $odc->status === 'maintenance' ? 'selected' : '' }}>Maintenance</option>
                                <option value="inactive" {{ $odc->status === 'inactive' ? 'selected' : '' }}>Nonaktif</option>
                            </select>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Catatan</label>
                        <textarea name="notes" class="form-control" rows="2">{{ $odc->notes }}</textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <select name="odc_id" class="form-select">
                            <option value="">Tanpa ODC</option>
                            @foreach($odcs as $odc)
                                <option value="{{ $odc->id }}" {{ $route->odc_id === $odc->id ? 'selected' : '' }}>{{ $odc->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
