...
@section('title', 'Pelanggan Existing')

@section('content')
<div class="page-header d-flex flex-wrap justify-content-between align-items-center">
    <div>
        <h2 class="mb-0"><i class="fa-solid fa-user-plus me-2" style="color:var(--primary);"></i>Pelanggan Existing</h2>
        <p class="section-subtitle mb-0 mt-1">Tambahkan pelanggan yang sudah ada ke sistem billing</p>
    </div>
    <div class="page-actions mt-2 mt-md-0">
        <a href="{{ route('customers.index') }}" class="btn btn-outline-premium px-4 py-2">
...
<div class="card shadow-sm border-0">
    <div class="card-header bg-white">
        <div class="d-flex align-items-center gap-2">
            ...
            <span>Formulir Pelanggan Existing</span>
        </div>
    </div>
    <div class="card-body p-4">
        <form action="{{ route('customer.store-existing') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
                <a href="{{ route('customers.index') }}" class="btn btn-light px-4">Batal</a>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const onuSearchInput = document.getElementById('onu_search');
    const onuResults = document.getElementById('onu_results');
    let onuTimer;

    onuSearchInput.addEventListener('input', function () {
        clearTimeout(onuTimer);
        const q = this.value.trim();
        if (q.length < 2) { onuResults.style.display = 'none'; return; }

        onuTimer = setTimeout(() => {
            fetch('{{ route("onu.available") }}?search=' + encodeURIComponent(q))
                .then(r => r.json())
                .then(data => {
                    onuResults.innerHTML = '';
                    if (!data.length) {
                        onuResults.innerHTML = '<div class="list-group-item text-muted" style="font-size:0.8rem;">Tidak ada ONU ditemukan</div>';
                        onuResults.style.display = 'block';
                        return;
                    }
                    data.forEach(onu => {
                        const btn = document.createElement('button');
                        ...
                        btn.addEventListener('click', function () {
                            ...
                            onuSearchInput.value = onu.serial_number || onu.caller_id || '';
                            onuResults.style.display = 'none';
                        });
                        onuResults.appendChild(btn);
                    });
                    onuResults.style.display = 'block';
                });
        }, 300);
    ...
    document.addEventListener('click', function (e) {
        if (!onuSearchInput.contains(e.target) && !onuResults.contains(e.target)) {
            onuResults.style.display = 'none';
        }
    });
});
