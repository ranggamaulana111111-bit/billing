...
| **Isolir** | AutoIsolir, SyncIsolirIps, MikrotikSetupIsolir | → Billing (overdue), → MikroTik (PPP profile) | derived dari `invoices` |
...
| **Automation** | AutomationController + `automation:*` | → OLT poll | `noc_automation_jobs` |
...
- Authorization dasar via middleware `IsAdmin`, `IsTeknisiOrAdmin`, `IsNoc`
...
| Reliability | Queue + retry untuk OLT poll & job async | ⚠️ Best-effort, belum chaos-tested |
...
- Queue job (`PollOltJob`) pakai database queue dengan retry default Laravel.
